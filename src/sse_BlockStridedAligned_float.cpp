__m128 _mm_load_ps(BlockStridedAligned<float> access) {
   return _mm_load_ps(&access[0]);
}
__m128 _mm_loadu_ps(BlockStridedAligned<float> access) {
   __m128 a0, a1, a2, a3;
...
void _mm_store_ps(BlockStridedAligned<float> access, __m128 x) {
   _mm_store_ps(&access[0], x);
}
void _mm_storeu_ps(BlockStridedAligned<float> access, __m128 x) {
